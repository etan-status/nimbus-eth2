...
type
  ...
  # https://github.com/ethereum/consensus-specs/blob/c9e0d02c2c343e522a0e4e9a574ce0d234918d73/specs/gloas/beacon-chain.md#executionpayloadenvelope
...
type
  ExecutionBranch* =
    List[Eth2Digest, Limit log2trunc(MaxLatestBlockHashGindex)]
...
func get_lc_execution_root*(
  ...
  if epoch >= cfg.DENEB_FORK_EPOCH:
    const BLOCK_HASH_GINDEX = get_generalized_index(
      deneb.ExecutionPayload, "block_hash")
    return merkle_branch_root(
      header.execution_block_hash,
      header.execution_branch.asSeq.toOpenArray(
        0, header.execution_branch.high - capella.ExecutionBranch.len),
      log2trunc(BLOCK_HASH_GINDEX), get_subtree_index(BLOCK_HASH_GINDEX))

  if epoch >= cfg.CAPELLA_FORK_EPOCH:
    const BLOCK_HASH_GINDEX = get_generalized_index(
      capella.ExecutionPayload, "block_hash")
    return merkle_branch_root(
      header.execution_block_hash,
      header.execution_branch.asSeq.toOpenArray(
        0, header.execution_branch.high - capella.ExecutionBranch.len),
...
func is_valid_light_client_header*(
  ...
  let
    epoch = header.beacon.slot.epoch
    execution_branch_len =
      if epoch >= cfg.GLOAS_FORK_EPOCH:
        log2trunc(LATEST_BLOCK_HASH_GINDEX_GLOAS)
      elif epoch >= cfg.DENEB_FORK_EPOCH:
        log2trunc(LATEST_BLOCK_HASH_GINDEX_DENEB)
      elif epoch >= cfg.CAPELLA_FORK_EPOCH:
        log2trunc(LATEST_BLOCK_HASH_GINDEX)
      else:
        0

  if header.execution_branch.len != execution_branch_len:
    return false

  if epoch >= cfg.GLOAS_FORK_EPOCH:
    return is_valid_merkle_branch(
      header.execution_block_hash, header.execution_branch.asSeq,
      log2trunc(LATEST_BLOCK_HASH_GINDEX_GLOAS),
      get_subtree_index(LATEST_BLOCK_HASH_GINDEX_GLOAS),
  ...
  if epoch >= cfg.CAPELLA_FORK_EPOCH:
    return is_valid_merkle_branch(
      ...
      header.execution_branch.asSeq.toOpenArray(
        header.execution_branch.len - capella.ExecutionBranch.len,
        header.execution_branch.high),
      log2trunc(capella.EXECUTION_PAYLOAD_GINDEX),
      get_subtree_index(capella.EXECUTION_PAYLOAD_GINDEX),
      header.beacon.body_root)

  header.execution_block_hash == ZERO_HASH
...
func upgrade_lc_header_to_gloas*(
  ...
  if epoch >= cfg.DENEB_FORK_EPOCH:
    ...
    return LightClientHeader(
      ...
      execution_branch: ExecutionBranch.init(
        @(pre.execution.build_proof(BLOCK_HASH_GINDEX).get) &
  ...
  if epoch >= cfg.CAPELLA_FORK_EPOCH:
    ...
    return LightClientHeader(
      ...
      execution_block_hash: pre.execution.block_hash,
      execution_branch: ExecutionBranch.init(
        @(execution_header.build_proof(BLOCK_HASH_GINDEX).get) &
        @(pre.execution_branch)))
